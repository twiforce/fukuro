...
function deletePost($id, $error_if_doesnt_exist=true, $rebuild_after=true) {
	...
	while ($post = $query->fetch(PDO::FETCH_ASSOC)) {
		...
		if (!$post['thread']) {
            ...
            // Delete JSON thread file
            file_unlink($board['dir'] . $config['dir']['res'] . $post['id'] . '.json');
...
function numPosts($id) {
	global $board;
	$query = prepare(sprintf("SELECT COUNT(*) AS `replies`, COUNT(NULLIF(`file`, NULL)) AS `images` FROM ``posts_%s`` WHERE `thread` = :thread", $board['uri'], $board['uri']));
...
function buildIndex() {
	...
	for ($page = 1; $page <= $config['max_pages']; $page++) {
		...
		if ($config['api']['enabled']) {
			...
			$json = json_encode($api->translatePage($threads), JSON_UNESCAPED_UNICODE);
	...
	// json api catalog
	if ($config['api']['enabled']) {
		$json = json_encode($api->translateCatalog($catalog), JSON_UNESCAPED_UNICODE);
...
function buildThread($id, $return = false, $mod = false) {
	...
	if ($config['api']['enabled']) {
		$api = new Api();
		$json = json_encode($api->translateThread($thread), JSON_UNESCAPED_UNICODE);
